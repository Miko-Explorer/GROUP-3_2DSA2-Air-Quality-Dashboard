...
for city, url in urls_pl.items():
    ...
    if response.status_code == 200:
        ...
        pols_idx = [div.text for div in pols_idx_divs]
        if len(pols_idx) >= 4:
            cities[city] = {"PM2.5": int(float(pols_idx[0].strip())), "PM10": int(float(pols_idx[1].strip())), "O3": int(float(pols_idx[3].strip())), "NO2": int(float(pols_idx[2].strip()))}
...
for city in sorted(cities.keys()):
    data = cities[city]
    final_data.append([city, date_string, time_string, data["PM2.5"], data["PM10"], data["O3"], data["NO2"]])

if len(final_data) > 1:
    sheet.clear()
    sheet.update(final_data, 'A1')
    print(f"Success! Updated {len(final_data)-1} cities at {time_string} on {date_string}.")
else:
